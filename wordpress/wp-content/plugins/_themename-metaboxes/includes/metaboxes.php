...
function _themename_post_metabox_html($post)
{
    ...
    wp_nonce_field(
        '_themename_update_post_metabox',
        '_themename_update_post_nonce'
    );
    ?>
    <p>
        <label for="_themename_post_metabox_html"><?php esc_html_e(
            'Post Subtitle',
            '_themename-_pluginname'
        ); ?></label>
        <br />
        <input class="widefat" type="text" name="_themename_post_subtitle_field" id="_themename_post_metabox_html" value="<?php echo esc_attr(
            $subtitle
        ); ?>" />
    </p>
    <p>
        <label for="_themename_post_layout_field"><?php esc_html_e(
            'Layout',
            '_themename-_pluginname'
        ); ?></label>
...
function _themename_save_post_metabox($post_id, $post)
{
    ...
    if (
        !isset($_POST['_themename_update_post_nonce']) ||
        !wp_verify_nonce(
            $_POST['_themename_update_post_nonce'],
            '_themename_update_post_metabox'
        )
    ) {
        return;
